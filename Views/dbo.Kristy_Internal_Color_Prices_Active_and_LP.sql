...
CREATE VIEW [dbo].[Kristy_Internal_Color_Prices_Active_and_LP]  as
select WWS.BREED, WWS.NEW, WWS.YS, case when BI.LP <> 0 THEN 'Y' ELSE '' END AS LP, BI.CODENO, WWS.REGNAME, WWS.CODENAME, WWS.D, CP.NEWCOST, CP.NEWSELL, 
CAST(((CP.NEWSELL - CP.NEWCOST) /  CP.NEWSELL)*100 AS DECIMAL(4,1)) AS GP, 
...
BI2.STYPE = 'X' AND BI2.ACTIVE = -1 AND BI2.CODENO NOT LIKE '514%' LEFT OUTER JOIN CSXDPRICE CPSXD ON BI2.CODENO = CPSXD.CODENO WHERE BI.STYPE = 'C' and (BI.ACTIVE <> 0 or BI.LP <> 0)
UNION ALL SELECT WWS.BREED, WWS.NEW, WWS.YS, case when BI.LP <> 0 THEN 'Y' ELSE '' END AS LP, BI.CODENO, WWS.REGNAME, WWS.CODENAME, WWS.D, NULL, NULL, NULL, NULL, NULL, NULL,NULL, NULL, 
