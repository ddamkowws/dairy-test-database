...
CREATE 	VIEW [dbo].[COLORQUREY_TABLE] as
...
case when [M].[SERV_SCR] Is Null then NULL ELSE Round([M].[SCR],1) END AS SCR, 
case when [M].[SERV_SCR] Is Null then NULL ELSE [M].[SERV_SCR] END AS SERV_SCR,
case when [M].[SERV_SCR] Is Null then NULL ELSE [M].[REL_SCR] END AS REL_SCR, 
...
WWS.S1 AS S1, WWS.S2 AS S2, WWS.S3 AS S3, WWS.S4 AS S4, WWS.S5 AS S5, WWS.S6 AS S6, WWS.S7 AS S7,
...
AS CEDATE, M.G_IND AS G_IND, 
CASE WHEN [WWS].[BREED]='JE' And [M].[G_IND] <> 0 THEN 'G' ELSE '' END AS GKEY, 
...
ATA_SSFS, M.ATA_STAR AS ATA_STAR, WWS.UDT AS UDT, BI2.STYPE AS STYPE, M.CM_DOL, M.FM_DOL, M.GM_DOL
FROM BULLINDEX AS BI INNER JOIN BI_CODENO BI2 ON BI.CODENO = BI2.CODENO
LEFT JOIN ecstatus AS EC ON BI2.EC = EC.EC 
LEFT JOIN Megadairy AS M ON BI.UID = M.UID 
JOIN WWSBULLS WWS ON BI.UID = WWS.UID
